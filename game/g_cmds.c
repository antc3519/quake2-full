...
void Cmd_Glider(edict_t* ent)
{
	if (ent->client->gliding == false)
		ent->client->gliding = true;
	else
		ent->client->gliding = false;
}
...
void Cmd_Grapple(edict_t* ent)
{
	...
	tr = gi.trace(start, NULL, NULL, end, ent, MASK_SOLID); // Trace the line
	if (tr.endpos && ent->client->grapples > 0) // Trace the line
	{
		VectorScale(forward, 500, forward); //Where to hit? Edit -5000 to whatever you like the push to be
		VectorAdd(forward, ent->velocity, ent->velocity); // Adding velocity vectors
		ent->client->grapples -= 1;
	}
	else
		gi.centerprintf(ent, "Out of grapples, land for more");
}

void Cmd_Recall(edict_t* ent) {
	VectorCopy(ent->s.origin, ent->client->resp.checkpointPosition);
}
...
void ClientCommand(edict_t* ent)
{
	...
	else if (Q_stricmp(cmd, "class1") == 0)
	{
		...
		gi.bprintf(PRINT_CHAT, "%s, is Class Rabbit", ent->client->pers.netname);
	}
	else if (Q_stricmp(cmd, "class2") == 0)
	{
		ent->client->resp.class = 2;
		PutClientInServer(ent);
		gi.bprintf(PRINT_CHAT, "%s, is Class Bear", ent->client->pers.netname);
	}
	else if (Q_stricmp(cmd, "class3") == 0)
	{
		ent->client->resp.class = 3;
		PutClientInServer(ent);
		gi.bprintf(PRINT_CHAT, "%s, is Class Cat", ent->client->pers.netname);
	}
